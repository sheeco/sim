...
**!!! ALL CASE SENSITIVE !!!** 
 ...
 -------------------------- | ---------------------------- 
  ...
  -continuous-trace     []  |  将轨迹文件中的离散点模拟成连续的折线轨迹（默认采用）；
 ...
 -------------------------- | ---------------------------- 
  ...
  -trans-window         []  |  单次数据传输的最大字节数（默认 2000）
  -node                 []  |  节点个数（默认 29）； 
  ...
  -energy               []  |  节点初始能量值（默认无限制）； 
                            ...
                            |    < 数据生成参数 > 
 -------------------------- | ---------------------------- 
  -data-rate            []  |  节点每秒生成的数据字节数（默认 200/30）； 
  -data-size            []  |  单个数据包的字节数（默认 200）； 
 ...
 -------------------------- | ---------------------------- 
  ...
  -pred-strict          []  |  Prophet 路由中节点投递概率相等时是否放弃转发（默认是）； 
