...
{
	cout<<">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Hotspot Classification"<<endl;
...
void HAR::SendData(int time)
{
	...
	for(vector<CMANode>::iterator iMANode = m_MANodes.begin(); iMANode != m_MANodes.end(); )
	{
		...
		do
		{
			...
			if( CBase::getDistance( *iMANode, *m_sink) <= TRANS_RANGE )
			{
				...
				{
					cout << " MA " << iMANode->getID() << " send " << iMANode->getBufferSize() << " data" << endl; 
				...
				if( iMANode->routeIsOverdue() )
				{
					...
					if( m_sink->hasMoreNewRoutes() )
					{
						...
						cout << " MA " << iMANode->getID() << " update route" << endl;
					}
					//FIXME: 若路线数目变少，删除多余的MA
					else
					{
						iMANode->setFlag(false);
						cout << " MA " << iMANode->getID() << " is deleted" << endl;
						break;
					}
				}
			}
					...
					&& iMANode->getWaitingTime() < 0 )
			{
				...
				else
				{
					...
					cout << " MA " << iMANode->getID() << " wait for " << tmp << endl;
				}
	...
	for(vector<CMANode>::iterator iMANode = m_MANodes.begin(); iMANode != m_MANodes.end(); iMANode++)
	{
		for(vector<CNode>::iterator inode = m_nodes.begin(); inode != m_nodes.end(); inode++)
		{
			...
			{
				cout << " MA " << iMANode->getID() << " receive " << inode->getBufferSize() << " data" << endl;
	...
	cout << "Delivery Ratio: " << CData::getDataArrivalCount() << " / " << CData::getDataCount() << endl;
}
...
void HAR::PrintInfo(int time)
{
	...
	{
		ofstream hotspot_details("hotspot-details.txt", ios::app);
		if(time == 0)
			hotspot_details << logInfo;
		hotspot_details << time << TAB << g_selectedHotspots.size() << endl; 
		//CFileParser::writeHotspotToFile("hotspot.txt", m_hotspots, false);

		//CFileParser::writeHotspotToFile("hotspot_detail.txt", m_hotspots, true);
		ofstream hotspot("hotspot.txt", ios::app);
		if(time == 0)
			hotspot << logInfo;
		CFileParser::writeHotspotStatisics(time, "hotspot.txt", m_hotspots);
		...
		if(time == 0)
			ed << logInfo;
		ed << time << TAB << calculateEDTime(time) << TAB << endl;
		ed.close();
	...
	if(time % SLOT_RECORD_INFO == 0)
	{
		...
		delivery_ratio << time << TAB << CData::getDataArrivalCount() << " / " << CData::getDataCount() << " = " << CData::getDeliveryRatio() << endl;
	...
	if(time % ( SLOT_HOTSPOT_UPDATE ) == 0)
	{
		...
		if(time == 0)
			delivery_ratio << logInfo;
		delivery_ratio << time << TAB << tmp_nDelivery << " / " << tmp_nData << " = " << (double)tmp_nDelivery / tmp_nData << endl;
		delivery_ratio.close();
		...
		|| time == RUNTIME)
	{
		...
		ofstream ma("ma-buffer.txt", ios::app);
		if(time == 0)
			ma << logInfo;
		ma << time << TAB;
		for(vector<CMANode>::iterator iMA = m_MANodes.begin(); iMA != m_MANodes.end(); iMA++)
			ma << iMA->getBufferSize() << "  " ;
		ma << endl;
		ma.close();

		ofstream node("node-buffer.txt", ios::app);
		if(time == 0)
			node << logInfo;
		node << time << TAB;
		for(vector<CNode>::iterator inode = m_nodes.begin(); inode != m_nodes.end(); inode++)
